...
### P2PKH - Quick recap
We learned that a **Stratis Address** was the **hash of a** **public key**:  

```cs
var publicKeyHash = new Key().PubKey.Hash;
var StratisAddress = publicKeyHash.GetAddress(Network.Main);
...
We also learned that as far as the blockchain is concerned, there is no such thing as a **Stratis address**. The blockchain identifies a receiver with a **ScriptPubKey**, and such **ScriptPubKey** could be generated from the address:  

```cs
var scriptPubKey = StratisAddress.ScriptPubKey;
...
```cs
var sameStratisAddress = scriptPubKey.GetDestinationAddress(Network.Main);
```

### P2PK

However, all **ScriptPubKey** does not represent a Stratis Address. For example the first transaction in the blockchain, called the genesis:  
...
var firstStratisAddressEver = firstScriptPubKeyEver.GetDestinationAddress(Network.Main);
...
A Stratis address is represented by: **OP_DUP OP_HASH160 &lt;hash&gt; OP_EQUALVERIFY OP_CHECKSIG**
...
These two types of payment are referred as **P2PK** (pay to public key) and **P2PKH** (pay to public key hash).
...
> ([Discussion](https://www.reddit.com/r/Bitcoin/comments/4isxjr/petition_to_protect_satoshis_coins/d30we6f)) If the early use of P2PK will not be addressed, it will have a serious impact on the Stratis price.  
